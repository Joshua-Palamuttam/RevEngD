...
/**
 * This class is a thin wrapper to help with setting up SOOT for whole-program
 * analysis. The API can be used by calling {@link #create()} followed by calls
 * to other config methods, such as, {@link #addDirectory(String)},
 * {@link #setEntryClass(String)}, and {@link #addEntryMethod(String)}, and then
 * finally calling {@link #build()} to acquire the {@link soot.Scene} object,
 * which has the result of the analysis. Here is a sample use:
 * 
 * <pre>
 * Scene scene = SceneBuilder.create().addDirectory(pathToBinOrSrcDir).setEntryClass("class.with.Main")
 * 		.addEntryPointMatcher(new MainMethodMatcher("class.with.Main"))
 * 		.addExclusions(Arrays.asList("java.*", "javax.*", "sun.*"))
 * 		.addExclusions(Arrays.asList("soot.*", "polygot.*")).addExclusions(Arrays.asList("org.*", "com.*")).build();
 * 
...
public class SceneBuilder {
	...
	/**
	 * Creates an empty SceneBuilder. SceneBuilder is a wrapper class for setting up
	 * soot for he whole-program analysis.
	 * 
	 * @return SceneBuilder
	 * @see #build()
	 */
	...
	/**
	 * Use it to process all java file or class files (including packages) in a
	 * directory. SOOT will try to load all of the classes (and dependent classes)
	 * in this directory. If the dependent classes are not found in the directory,
	 * then SOOT will look for them in the supplied class path.
	 * 
	 * @param dir
	 *            A directory to be loaded in SOOT.
	 * @return {@link SceneBuilder} The builder object being used to build a
	 *         {@link Scene}.
	...
	/**
	 * Use it to process all java file or class files (including packages) in the
	 * supplied directories. SOOT will try to load all of the classes (and dependent
	 * classes) in these directories. If the dependent classes are not found in the
	 * directories, then SOOT will look for them in the supplied class path.
	 * 
	 * @param dirs
	 *            A collection of directories to be loaded in SOOT.
	 * @return {@link SceneBuilder} The builder object being used to build a
	 *         {@link Scene}.
	 * @see {@link #addClassPath(String)}, {@link #addClassPaths(Collection)}
	 */
	...
	/**
	 * The {@link SceneBuilder} API is designed to help setup SOOT for the whole
	 * program analysis. The whole-program analysis needs a class(s) with the
	 * <tt>main</tt> method as the starting point. Use this method to set the main
	 * class.
	 * 
	 * @param clazz
	 *            The fully-qualified name of the main class. e.g.,
	 *            <tt>my.package.MainClass</tt>
	 * @return {@link SceneBuilder} The builder object being used to build a
	 *         {@link Scene}.
	 */
	...
	/**
	 * Adds an {@link IEntryPointMatcher} to {@link SceneBuilder}. The entry points
	 * ({@link SootMethod}s) is used to construct call-graph.
	 * 
	 * @param matcher
	 *            An {@link IEntryPointMatcher} to be added
	 * @return {@link SceneBuilder} The builder object being used to build a
	 *         {@link Scene}.
	 */
	...
	/**
	 * Adds a collection of {@link IEntryPointMatcher} to {@link SceneBuilder}. The
	 * entry points ({@link SootMethod}s) is used to construct call-graph.
	 * 
	 * @param matchers
	 *            A collection of {@link IEntryPointMatcher} to be added
	 * @return {@link SceneBuilder} The builder object being used to build a
	 *         {@link Scene}.
	 */
	...
	/**
	 * The whole-program analysis can run slow if the user-defined classes import
	 * some of the default JDK classes. If the analysis does not need the JDKs
	 * classes or for that matter classes from dependent libraries, then they can be
	 * excluded from analysis using this method. Doing so will improve SOOT's
	 * performance.
	 * 
	 * @param exclusion
	 *            The libraries (pattern) to be excluded. e.g.,
	 *            <tt>java.* or javax.*</tt>
	 * @return {@link SceneBuilder} The builder object being used to build a
	 *         {@link Scene}.
	 */
	 ...
	 * 
	 * @param exclusions
	 *            A collection of libraries to be included.
	 * @return {@link SceneBuilder} The builder object being used to build a
	 *         {@link Scene}.
	...
	/**
	 * Configures SOOT for whole-program analysis based on the intermediate
	 * configuration method calls. Runs all of the SOOT analysis and finally,
	 * returns the {@link soot.Scene} object, which contains the result of the
	 * analysis.
	 * 
	 * @return
	 */
	...
	private List<SootMethod> computeEntryPoints(Scene scene) {
		final List<SootMethod> methods = new ArrayList<SootMethod>();
